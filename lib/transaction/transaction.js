...
var FLUXNODE_INTERNAL_NORMAL_TX_VERSION = 1;
var FLUXNODE_INTERNAL_P2SH_TX_VERSION = 2;

var FLUXNODE_TX_VERSION_5 = 5;
var FLUXNODE_TX_VERSION_6 = 6;
...
if (this.version == FLUXNODE_TX_VERSION_5) {
  writer.writeUInt8(this.zelnodeData.nType);
  if (this.zelnodeData.nType == ZELNODE_START_TX_TYPE) {
    writer.write(this.zelnodeData.collateralOutHash);
    writer.writeUInt32LE(this.zelnodeData.collateralOutIndex);
    writer.writeVarintNum(this.zelnodeData.collateralPublicKey.length);
    writer.write(this.zelnodeData.collateralPublicKey);
    writer.writeVarintNum(this.zelnodeData.publicKey.length);
    writer.write(this.zelnodeData.publicKey);
    writer.writeUInt32LE(this.zelnodeData.sigTime);
    writer.writeVarintNum(this.zelnodeData.sig.length);
    writer.write(this.zelnodeData.sig);
  }
  else if (this.zelnodeData.nType == ZELNODE_CONFIRM_TX_TYPE) {
    writer.write(this.zelnodeData.collateralOutHash);
    writer.writeUInt32LE(this.zelnodeData.collateralOutIndex);
    writer.writeUInt32LE(this.zelnodeData.sigTime);
    writer.writeUInt8(this.zelnodeData.benchmarkTier);
    writer.writeUInt32LE(this.zelnodeData.benchmarkSigTime);
    writer.writeUInt8(this.zelnodeData.nUpdateType);
    writer.writeVarintNum(this.zelnodeData.ipaddress.length);
    writer.write(this.zelnodeData.ipaddress);
    writer.writeVarintNum(this.zelnodeData.sig.length);
    writer.write(this.zelnodeData.sig);
    writer.writeVarintNum(this.zelnodeData.benchmarkSig.length);
    writer.write(this.zelnodeData.benchmarkSig);
  }
} else if (this.version == FLUXNODE_TX_VERSION_6) {
  writer.writeUInt8(this.zelnodeData.nType);
  if (this.zelnodeData.nType == ZELNODE_START_TX_TYPE) {
    writer.writeUInt32LE(this.zelnodeData.nFluxTxVersion);
    if (this.zelnodeData.nFluxTxVersion == FLUXNODE_INTERNAL_NORMAL_TX_VERSION) {
      writer.write(this.zelnodeData.collateralOutHash);
      writer.writeUInt32LE(this.zelnodeData.collateralOutIndex);
      writer.writeVarintNum(this.zelnodeData.collateralPublicKey.length);
      writer.write(this.zelnodeData.collateralPublicKey);
      ...
      writer.writeVarintNum(this.zelnodeData.sig.length);
      writer.write(this.zelnodeData.sig);
    } else if (this.zelnodeData.nFluxTxVersion == FLUXNODE_INTERNAL_P2SH_TX_VERSION) {
      writer.write(this.zelnodeData.collateralOutHash);
      writer.writeUInt32LE(this.zelnodeData.collateralOutIndex);
      writer.writeVarintNum(this.zelnodeData.publicKey.length);
      writer.write(this.zelnodeData.publicKey);
      writer.writeVarintNum(this.zelnodeData.P2SHRedeemScript.length);
      writer.write(this.zelnodeData.P2SHRedeemScript);
      writer.writeUInt32LE(this.zelnodeData.sigTime);
      writer.writeVarintNum(this.zelnodeData.sig.length);
      writer.write(this.zelnodeData.sig);
    }
...
Transaction.prototype.fromBufferReader = function(reader) {
  ...
  if (this.version == FLUXNODE_TX_VERSION_5) {
    this.isZelNodeTX = true;
    this.zelnodeData.nType = reader.readUInt8();
    if (this.zelnodeData.nType == ZELNODE_START_TX_TYPE) {
      this.zelnodeData.collateralOutHash = reader.read(32);
      this.zelnodeData.collateralOutIndex = reader.readUInt32LE();
      var sliceByte = reader.readVarintNum(); // == 21 in hex = 33 usually
      this.zelnodeData.collateralPublicKey = reader.read(sliceByte);
      var sliceByte2 = reader.readVarintNum(); // == 41 = 65 usually
      this.zelnodeData.publicKey = reader.read(sliceByte2);
      this.zelnodeData.sigTime = reader.readUInt32LE();
      var sizeSig = reader.readVarintNum();
      this.zelnodeData.sig = reader.read(sizeSig);
    }
    if (this.zelnodeData.nType == ZELNODE_CONFIRM_TX_TYPE) {
      this.zelnodeData.collateralOutHash = reader.read(32);
      this.zelnodeData.collateralOutIndex = reader.readUInt32LE();
      this.zelnodeData.sigTime = reader.readUInt32LE();
      this.zelnodeData.benchmarkTier = reader.readUInt8();
      this.zelnodeData.benchmarkSigTime = reader.readUInt32LE();
      this.zelnodeData.nUpdateType = reader.readUInt8();
      var sizeIP = reader.readVarintNum();
      this.zelnodeData.ipaddress = reader.read(sizeIP);
      var sizeSig = reader.readVarintNum();
      this.zelnodeData.sig = reader.read(sizeSig);
      var sizeBenchmarkSig = reader.readVarintNum();
      this.zelnodeData.benchmarkSig = reader.read(sizeBenchmarkSig);
    }
    return this;
  } else if (this.version == FLUXNODE_TX_VERSION_6) {
    this.isZelNodeTX = true;
    this.zelnodeData.nType = reader.readUInt8();
    if (this.zelnodeData.nType == ZELNODE_START_TX_TYPE) {
      this.zelnodeData.nFluxTxVersion = reader.readUInt32LE();
      if (this.zelnodeData.nFluxTxVersion == FLUXNODE_INTERNAL_NORMAL_TX_VERSION) {
        this.zelnodeData.collateralOutHash = reader.read(32);
        this.zelnodeData.collateralOutIndex = reader.readUInt32LE();
        var sliceByte = reader.readVarintNum(); // == 21 in hex = 33 usually
        ...
        this.zelnodeData.collateralOutHash = reader.read(32);
        this.zelnodeData.collateralOutIndex = reader.readUInt32LE();
        var sliceByte1 = reader.readVarintNum(); // == 41 = 65 usually
        this.zelnodeData.publicKey = reader.read(sliceByte1);
        var sizeRedeemScript = reader.readVarintNum();
        this.zelnodeData.P2SHRedeemScript = reader.read(sizeRedeemScript);
        this.zelnodeData.sigTime = reader.readUInt32LE();
        var sizeSig = reader.readVarintNum();
        this.zelnodeData.sig = reader.read(sizeSig);
      }
...
Transaction.prototype.toObject = Transaction.prototype.toJSON = function toObject() {
  ...
  obj.zelnodeData = this.zelnodeData;
  obj.isZelNodeTX = true;
}
  return obj;
};
...
if (this.version != 5 && this.version != 6) {
  ...
  this.isZelNodeTX = true;
  this.zelnodeData = transaction.zelnodeData;
}
